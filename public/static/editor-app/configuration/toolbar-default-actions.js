...
KISBPM.TOOLBAR = {
    ACTIONS: {
        ...
        //         backdrop: true,
        //         keyboard: true,
        //         template: 'editor-app/popups/deploy-model.html?version=' + Date.now(),
        //         scope: services.$scope
        //     });
        // },
...
var SaveModelCtrl = [ '$rootScope', '$scope', '$http', '$route', '$location',
    ...
    $scope.save = function (successCallback) {
      ...
      //解决ie保存报错
       ...
       var params = {
        ...
        var token = window.sessionStorage.getItem('Csrf-Token');
        // Update
        $http({    method: 'POST',
            data: params,
            ignoreErrors: true,
    ...
    $scope.deploy = function (successCallback) {
        ...
        var json = $scope.editor.getJSON();
        if (json.properties.process_id == "未定义") {
            $scope.status.loading = false;
            $scope.error = {};
            $scope.error.deployModel = true;
            return;
        }
        json = JSON.stringify(json);
