      ...
      <template slot-scope="scope">
        <div class="tree_content">
          <div class="tree_item">
            ...
            <span v-else-if="treeType === 'department'" class="iconfont icon-bumenguanli" />
            <span v-else class="iconfont" :class="scope.data.children || scope.data.type === 'org' ? 'icon-bumenguanli' : 'icon-avatar1'" />
...
export default {
  ...
  methods: {
    ...
    async __getList() {
      const res = await getOrgTree('', this.treeType, false)
      // console.log(res)
