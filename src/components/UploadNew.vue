<template>
  <div>
    <el-dialog
      title="上传"
      :visible.sync="isShow"
      class="content-dialog-box upload-dialog specialPop"
    ...
    >
      ...
      <div v-preventReClick class="upload-btn" @click="submitUpload">
        ...
        <div class="detail-info">
          <ul class="filelist">
            <div class="scroll-info-list">
              ...
              <li v-if="errortwo" class="info-err">
                {{ errortwo }}
              </li>
...
export default {
  ...
  methods: {
    ...
    checkTypeAndSize(file) {
      ...
      if (!isLt10M) {
        this.errorDetails.push('上传文件大小不能超过 30MB！')
          ...
          if (this.two) {
            ...
            var setTime = setInterval(function() {
              ...
              if (countDown <= 0) {
                ...
                console.log(that.errorDetails.length)
                if ((!that.resultInfo) && (!that.errorDetails.length)) {
                  that.errortwo = '系统错误，请联系管理员！'
                }
              } else {
                countDown--
              }
    ...
    uploadMessage(files = [], res, status = true) {
      this.resetForm2()
      const { success, messages, message } = res
      if (status) {
        if (!success) {
          if (messages && messages.length) {
            this.errorDetails = [...messages.map(item => {
              ...
              return `${this.$t(item.messageId)}`
            })]
            console.log('111111111111111111', this.errorDetails)
          } else {
            console.log('message2222222', message)
