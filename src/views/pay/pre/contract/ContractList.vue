        ...
        <el-form ref="form" class="search-form default-input" label-position="left" :model="searchParam.params">
          ...
          <!-- <el-form-item>
            <el-row type="flex" :gutter="5" justify="space-between">
              ...
              <el-col :span="16">
                <el-select v-model="searchParam.params.prewithdrawType" placeholder="请选择" clearable>
                  <el-option
          ...
          <el-form-item prop="identiferNum">
            <el-row type="flex" :gutter="5" justify="space-between">
              <el-col :span="8">
                {{ $t('pay.pre.contract.identiferNum') }}:
              </el-col>
              <el-col :span="16">
        ...
        <div ref="tables" style="height: 100%;">
          ...
          >
            <el-table-column
              ...
              width="55"
            />
            <el-table-column prop="identiferNum" :label="$t('pay.pre.contract.identiferNum')" min-width="100" align="center">
              <template slot-scope="scope">
                <router-link :to="{path:'/pay/pre/contract/detail',query:{id:scope.row.id}}">
                  {{ scope.row.identiferNum }}
                </router-link>
              </template>
            </el-table-column>
            <!-- <el-table-column prop="prewithdrawType" :label="$t('pay.pre.contract.prewithdrawType')" min-width="110" align="center" /> -->
            <el-table-column prop="contractIdentiferNum" :label="$t('pay.pre.contract.contractIdentiferNum')" min-width="120" align="center" />
            <el-table-column prop="subjectNum" :label="$t('pay.pre.contract.subjectNum')" min-width="110" align="center" />
            <el-table-column prop="subjectName" :label="$t('pay.pre.contract.subjectName')" min-width="150" align="center" show-overflow-tooltip />
            <el-table-column prop="localChainNum" :label="$t('pay.pre.contract.localChainNum')" min-width="120" align="center" />
            <el-table-column prop="localChainName" :label="$t('pay.pre.contract.localChainName')" min-width="150" align="center" show-overflow-tooltip />
            <el-table-column prop="salesOfficeNum" :label="$t('pay.pre.contract.salesOfficeNum')" min-width="120" align="center" />
            <el-table-column prop="subBrandNum" :label="$t('pay.pre.contract.subBrandNum')" min-width="120" align="center" />
            <el-table-column prop="subBrandName" :label="$t('pay.pre.contract.subBrandName')" min-width="150" align="center" show-overflow-tooltip />
            <el-table-column prop="prewithdrawUses" :label="$t('pay.pre.contract.prewithdrawUses')" min-width="150" align="center" show-overflow-tooltip />
            <el-table-column :label="$t('pay.pre.contract.prewithdrawAmount')" min-width="110" align="right">
              <template slot-scope="scope">
                {{ scope.row.prewithdrawAmount | numFormat(2) }}
              </template>
            </el-table-column>
            <el-table-column :label="$t('pay.pre.motion.clearedAmount')" min-width="110" align="right">
              <template slot-scope="scope">
                {{ scope.row.clearedAmount | numFormat(2) }}
              </template>
            </el-table-column>
            <el-table-column :label="$t('pay.pre.motion.diffAmount')" min-width="110" align="right">
              <template slot-scope="scope">
                {{ scope.row.diffAmount | numFormat(2) }}
              </template>
            </el-table-column>
            <el-table-column prop="planUseDate" :label="$t('pay.pre.contract.planUseDate')" min-width="110" align="center" />
          </el-table>
        </div>
      </div>
...
export default {
  ...
  methods: {
    ...
    handleCreate() {
      this.$router.push('/pay/pre/contract/edit')
    },
    handleCopy() {
      if (this.multipleSelection && this.multipleSelection.length === 1) {
        this.multipleSelection.forEach((rowData) => {
          this.$router.push({ path: '/pay/pre/contract/edit', query: { id: rowData.id, copy: true }})
        })
      } else {
        this.$message({
          type: 'info',
    ...
    handleEdit() {
      if (this.multipleSelection && this.multipleSelection.length === 1) {
        this.multipleSelection.forEach((rowData) => {
          ...
          this.$router.push({ path: '/pay/pre/contract/edit', query: { id: rowData.id }})
        })
      } else {
        this.$message({
          type: 'info',
