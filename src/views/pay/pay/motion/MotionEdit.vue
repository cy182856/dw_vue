    ...
    <div class="detail">
      ...
      <el-collapse v-model="activeNames">
        ...
        <el-collapse-item name="3">
          ...
          <div class="input_box">
            <div class="flex3">
              ...
              <div class="input">
                <milli-input v-model="payment.expenseAmount" to-fixed-num="2" />
              </div>
            </div>
            <div class="flex3">
              <div class="title">
              ...
              <div class="input">
                <milli-input v-model="payment.expenseInvoice" to-fixed-num="2" />
              </div>
            </div>
            <div class="flex3">
              <div class="title">
              ...
              <div class="input">
                <milli-input v-model="payment.expenseTax" to-fixed-num="2" />
              </div>
            </div>
          </div>
          <div class="input_box">
            <div class="flex3">
              ...
              <div class="input">
                <milli-input v-model="payment.expenseInvoiceTotal" to-fixed-num="2" />
              </div>
            </div>
            <div class="flex3">
              <div class="title">
              ...
              <div class="input">
                <milli-input v-model="payment.expenseWithdrawalTotal" to-fixed-num="2" />
              </div>
            </div>
            <div class="flex3">
              <div class="title">
              ...
              <div class="input">
                <milli-input v-model="payment.allocationDetails" to-fixed-num="2" />
              </div>
            </div>
          </div>
        </el-collapse-item>
        <el-collapse-item name="4">
          ...
          <div v-if="payment.attachmentAddStatus==='有发票'" class="add_table_row titlemin">
            <div>
              发票及税金信息
            </div>
            <div class="input">
          ...
          <div v-if="payment.attachmentAddStatus==='有发票'" class="input_box linemin">
            <div class="flex3">
              <div class="title">
                不含税金额合计
              </div>
          ...
          <div class="add_table_row titlemin">
            <div>预提明细</div>
            <el-button class="editbtn" size="mini" @click.stop="handleAddWthholding">
              新增
            </el-button>
          </div>
          <el-table
            ref="table"
            :data="payment.wthholdingDetails"
            style="margin-bottom: 20px;"
            :header-cell-style="{'text-align':'center'}"
            :cell-style="{'text-align':'center'}"
            border
          ...
          >
            ...
            <el-table-column label="原预提单编号" min-width="170">
              <template slot-scope="scope">
                <el-input v-model="scope.row.wthholdingCode" clearable />
              </template>
            </el-table-column>
            <el-table-column label="预提总金额" min-width="170">
              <template slot-scope="scope">
                <el-input v-model="scope.row.wthholdingAccount" clearable to-fixed-num="2" />
              </template>
            </el-table-column>
            <el-table-column label="预提用途" min-width="170">
              <template slot-scope="scope">
                <el-input v-model="scope.row.wthholdingName" clearable />
              </template>
            </el-table-column>
          </el-table>
          <div class="input_box linemin">
            <div class="flex3">
              <div class="title">
                清预提总金额
              </div>
              <div class="input">
                <milli-input v-model="wthholdingSum" to-fixed-num="2" :disabled="true" />
              </div>
            </div>
          </div>
          ...
          >
            ...
            <el-table-column label="提案编号" min-width="170">
              <template slot-scope="scope">
            ...
            <el-table-column label="Sub品牌编号" min-width="170">
              <template slot-scope="scope">
                <el-input v-model="scope.row.subCode" clearable />
              </template>
            </el-table-column>
            <el-table-column label="Sub品牌名称" min-width="170">
              <template slot-scope="scope">
                <el-input v-model="scope.row.subName" clearable />
              </template>
            </el-table-column>
            <el-table-column label="SalesOffice编号" min-width="170">
              <template slot-scope="scope">
                <el-input v-model="scope.row.salesOfficeCode" clearable />
              </template>
            </el-table-column>
            <el-table-column label="CG5编号" min-width="170">
              <template slot-scope="scope">
                <el-input v-model="scope.row.CG5Code" clearable />
              </template>
            </el-table-column>
            <el-table-column label="费用方编号" min-width="170">
              <template slot-scope="scope">
                <el-input v-model="scope.row.costCode" clearable />
              </template>
            </el-table-column>
            <el-table-column label="费用方名称" min-width="170">
              <template slot-scope="scope">
                <el-input v-model="scope.row.costName" clearable />
              </template>
            </el-table-column>
            <el-table-column label="成本中心编码" min-width="170">
              <template slot-scope="scope">
                <el-input v-model="scope.row.costCenterCOde" clearable />
              </template>
            </el-table-column>
            <el-table-column label="金额" min-width="170">
              <template slot-scope="scope">
                <milli-input v-model="scope.row.costAmount" to-fixed-num="2" />
              </template>
            </el-table-column>
          </el-table>
          <div class="input_box">
...
export default {
  ...
  data() {
    return {
      ...
      payment: {
        ...
        expenseTax: '',
        expenseInvoiceTotal: '',
        expenseWithdrawalTotal: '',
        allocationDetails: ''
      },
      fileList: [],
      processDialog: false,
      paymentType: [{ value: '活动费支付', lable: '01' }, { value: '合同费支付', lable: '02' }],
  ...
  computed: {
    ...
    divideAmountSum: function() {
      if (this.payment.details && this.payment.details.length > 0) {
        const sum = this.payment.details.map(
          (item) => (!item.costAmount ? 0 : parseFloat(item.costAmount)),
          2
        )
        return sum.reduce((sum, item) => sum + item)
      }
    ...
    wthholdingSum: function() {
      if (this.payment.wthholdingDetails && this.payment.wthholdingDetails.length > 0) {
        const sum = this.payment.wthholdingDetails.map(
          (item) => (!item.wthholdingAccount ? 0 : parseFloat(item.wthholdingAccount)),
          2
        )
        return sum.reduce((sum, item) => sum + item)
      }
  ...
  async created() {
    if (this.id) {
      ...
      this.payment.paymentType = '活动费支付'
  ...
  methods: {
    ...
    handleAddWthholding() {
      this.payment.wthholdingDetails.push({
        wthholdingCode: '',
        wthholdingAccount: '',
        wthholdingName: ''
      })
