        ...
        <el-form ref="form" class="search-form default-input" label-position="left" :model="searchParam.params">
          <el-form-item>
            <el-row type="flex" :gutter="5" justify="space-between">
              <el-col :span="8">
                {{ $t('pay.pay.motion.payIdentiferNum') }}:
              </el-col>
              <el-col :span="16">
      ...
      <div class="table-bg-box search-list-table">
        <div ref="tables" style="height: 100%;">
          ...
          >
            ...
            <el-table-column prop="monitorNo" :label="$t('pay.pay.motion.monitorNo')" min-width="120" align="center">
              <template slot-scope="scope">
                <router-link :to="{path:'/pay/pay/promoter/detail',query:{id:scope.row.id}}">
                  {{ scope.row.monitorNo }}
                </router-link>
              </template>
            </el-table-column>
            <el-table-column prop="preIdentiferNum" :label="$t('pay.pay.motion.preIdentiferNum')" min-width="150" align="center" />
            <el-table-column prop="subjectNum" :label="$t('pay.pay.motion.motionIdentiferNum')" min-width="150" align="center" />
            <el-table-column prop="subjectName" :label="$t('pay.pay.motion.subjectNum')" align="center" min-width="150" show-overflow-tooltip />
            <el-table-column prop="subjectName" :label="$t('pay.pay.motion.subjectName')" min-width="150" align="center" show-overflow-tooltip />
            <el-table-column prop="subBrandNum" :label="$t('pay.pay.motion.subBrandNum')" min-width="150" align="center" show-overflow-tooltip />
            <el-table-column prop="subBrandName" :label="$t('pay.pay.motion.subBrandName')" min-width="150" align="center" show-overflow-tooltip />
            <el-table-column prop="cg5Num" :label="$t('pay.pay.motion.cg5Num')" min-width="150" align="center" show-overflow-tooltip />
            <el-table-column prop="expenseSideNum" :label="$t('pay.pay.motion.expenseSideNum')" min-width="150" align="center" show-overflow-tooltip />
            <el-table-column prop="expenseSideName" :label="$t('pay.pay.motion.expenseSideName')" min-width="150" align="center" show-overflow-tooltip />
            <el-table-column prop="costCenterNum" :label="$t('pay.pay.motion.costCenterNum')" min-width="150" align="center" show-overflow-tooltip />
            <el-table-column :label="$t('pay.pay.motion.amount')" min-width="110" align="right">
              <template slot-scope="scope">
                {{ scope.row.amount | numFormat(2) }}
              </template>
            </el-table-column>
            <el-table-column :label="$t('pay.pay.motion.clearedAmount')" min-width="110" align="right">
              <template slot-scope="scope">
                {{ scope.row.clearedAmount | numFormat(2) }}
              </template>
            </el-table-column>
            <el-table-column :label="$t('pay.pay.motion.diffAmount')" min-width="110" align="right">
              <template slot-scope="scope">
                {{ scope.row.diffAmount | numFormat(2) }}
              </template>
            </el-table-column>
            <el-table-column prop="clearingDate" :label="$t('pay.pay.motion.clearingDate')" min-width="150" align="center" show-overflow-tooltip />
            <el-table-column prop="financialCompleDate" :label="$t('pay.pay.motion.financialCompleDate')" min-width="150" align="center" show-overflow-tooltip />
          </el-table>
        </div>
      </div>
...
export default {
  ...
  methods: {
    ...
    handleCreate() {
      this.$router.push('/pay/pay/promoter/edit')
    },
    handleCopy() {
      if (this.multipleSelection && this.multipleSelection.length === 1) {
        this.multipleSelection.forEach((rowData) => {
          this.$router.push({ path: '/pay/pay/promoter/edit', query: { id: rowData.id, copy: true }})
        })
      } else {
        this.$message({
          type: 'info',
    ...
    handleEdit() {
      if (this.multipleSelection && this.multipleSelection.length === 1) {
        this.multipleSelection.forEach((rowData) => {
          ...
          this.$router.push({ path: '/pay/pay/promoter/edit', query: { id: rowData.id }})
        })
      } else {
        this.$message({
          type: 'info',
