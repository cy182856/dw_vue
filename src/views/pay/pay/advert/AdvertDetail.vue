<template>
  <!-- 详细画面 -->
  <div class="summary">
    <advert-info :id="id" :key="id" :type="action?action:'detail'" @load="handleLoad" />
    <div class="detail">
...
export default {
  ...
  methods: {
    ...
    handleCopy() {
      this.$router.push({ path: '/pay/pay/advert/edit', query: { id: this.id, copy: true }})
    },
    handleEdit() {
      this.$router.push({ path: '/pay/pay/advert/edit', query: { id: this.id }})
    },
    handleDelete() {
      this.$confirm(this.$t('comm.tip9'), this.$t('comm.tips'), {
        ...
        // if (res && res.success) {
        //   this.$router.push('/pay/pay/advert/list')
        // }
