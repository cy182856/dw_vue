        ...
        <el-form ref="form" class="search-form default-input" label-position="left" :model="searchParam.params">
          <el-form-item>
            <el-row type="flex" :gutter="5" justify="space-between">
              <el-col :span="8">
                {{ $t('pay.pay.contract.payIdentiferNum') }}:
              </el-col>
              <el-col :span="16">
          ...
          <el-form-item>
            <el-row type="flex" :gutter="5" justify="space-between">
              <el-col :span="8">
                {{ $t('pay.pay.contract.applyUserNum') }}:
              </el-col>
              <el-col :span="16">
        ...
        <div ref="tables" style="height: 100%;">
          ...
          >
            <el-table-column
              ...
              width="55"
            />
            <el-table-column prop="monitorNo" :label="$t('pay.pay.contract.monitorNo')" min-width="120" align="center">
              <template slot-scope="scope">
                <router-link :to="{path:'/pay/pay/contract/detail',query:{id:scope.row.id}}">
                  {{ scope.row.monitorNo }}
                </router-link>
              </template>
            </el-table-column>
            <el-table-column prop="preIdentiferNum" :label="$t('pay.pay.contract.preIdentiferNum')" min-width="150" align="center" />
            <el-table-column prop="subjectNum" :label="$t('pay.pay.contract.subjectNum')" min-width="150" align="center" />
            <el-table-column prop="subjectName" :label="$t('pay.pay.contract.subjectName')" align="center" min-width="150" show-overflow-tooltip />
            <el-table-column prop="localChainNum" :label="$t('pay.pay.contract.localChainNum')" min-width="150" align="center" show-overflow-tooltip />
            <el-table-column prop="salesOfficeNum" :label="$t('pay.pay.contract.salesOfficeNum')" min-width="150" align="center" show-overflow-tooltip />
            <el-table-column prop="expenseSideNum" :label="$t('pay.pay.contract.expenseSideNum')" min-width="150" align="center" show-overflow-tooltip />
            <el-table-column prop="expenseSideName" :label="$t('pay.pay.contract.subjectName')" min-width="150" align="center" show-overflow-tooltip />
            <el-table-column :label="$t($t('pay.pay.contract.amount'))" min-width="110" align="right">
              <template slot-scope="scope">
                {{ scope.row.amount | numFormat(2) }}
              </template>
            </el-table-column>
            <el-table-column prop="financialCompleDate" :label="$t('pay.pay.contract.financialCompleDate')" min-width="150" align="center" show-overflow-tooltip />
            <el-table-column prop="clearingDate" :label="$t('pay.pay.contract.clearingDate')" min-width="150" align="center" show-overflow-tooltip />
          </el-table>
        </div>
      </div>
...
export default {
  ...
  methods: {
    ...
    handleCreate() {
      this.$router.push('/pay/pay/contract/edit')
    },
    handleCopy() {
      if (this.multipleSelection && this.multipleSelection.length === 1) {
        this.multipleSelection.forEach((rowData) => {
          this.$router.push({ path: '/pay/pay/contract/edit', query: { id: rowData.id, copy: true }})
        })
      } else {
        this.$message({
          type: 'info',
    ...
    handleEdit() {
      if (this.multipleSelection && this.multipleSelection.length === 1) {
        this.multipleSelection.forEach((rowData) => {
          ...
          this.$router.push({ path: '/pay/pay/contract/edit', query: { id: rowData.id }})
        })
      } else {
        this.$message({
          type: 'info',
