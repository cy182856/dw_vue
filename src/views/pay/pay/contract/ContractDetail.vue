<template>
  <!-- 详细画面 -->
  <div class="summary">
    <contract-info :id="id" :key="id" :type="action?action:'detail'" @load="handleLoad" />
    <div class="detail">
      <ul class="operation-box dialog-btn-box dialog-btn-box-detail">
        ...
        <!-- 编辑 -->
        <li v-if="payment.workflowFlag === '0' && payment.lastFlag === 1" class="operation-item" @click="handleEdit()">
        ...
        <!-- 审批履历 -->
        <li v-if="payment.workflowProcessInstId" class="operation-item" @click="approveHisDialog=true">
    ...
    <div v-if="approveHisDialog" class="approveHis">
      <process-detail :key="payment.workflowProcessInstId" :is-show="approveHisDialog" :title="$t('component.taskApprove.detailedProcess') + payment.workflowProcessInstId" :process-inst-id="payment.workflowProcessInstId" @close="approveHisDialog=false" />
    </div>
    ...
    <div class="versionDialog">
      ...
      >
        ...
        <version-his :key="payment.identiferNum" func-div="payment" :identifer-num="payment.identiferNum" @versionClick="versionDialog=false" />
      </el-drawer>
    </div>
  </div>
</template>
<script>
// import { deletePayment, cancelProcess, deleteApply } from '@/api/mrk/account.js'
import { names } from '@/mixins/names.js'
import { permission } from '@/mixins/permission-mixin.js'
import ContractInfo from '@/views/pay/pay/contract/components/ContractInfo'
export default {
  name: 'ContractDetail',
  ...
  methods: {
    ...
    handleCopy() {
      this.$router.push({ path: '/pay/pay/contract/edit', query: { id: this.id, copy: true }})
    },
    handleEdit() {
      this.$router.push({ path: '/pay/pay/contract/edit', query: { id: this.id }})
    },
    handleDelete() {
      this.$confirm(this.$t('comm.tip9'), this.$t('comm.tips'), {
        ...
        this.payment.submitType = 'submit'
        // const res = await deletPpayment([this.payment])
        // if (res && res.success) {
        //   this.$router.push('/pay/pay/contract/list')
        // }
    ...
    handleSubmit() {
      this.payment.submitType = 'deleteReSubmit'
