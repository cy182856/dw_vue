<template>
  ...
  <div class="summary">
    ...
    <div class="detail">
      <div class="pagetitle">
        <span class="pagespan">{{ $t('sale.salesorder.identiferNum') }} : {{ salesorder.identiferNum }} {{ $t('comm.version') }} : {{ salesorder.version }}</span>
      </div>
          ...
          <div v-else-if="item.elCollapseItemType === 'customize'">
            <div class="total-content">
              ...
              <div class="item bg-c2">
                ...
                <div class="center">
                  <milli-input v-model="salesorder.productTax" :disabled="true" :to-fixed-num="2" class="input" />
                </div>
              </div>
              <div class="item bg-c1">
                <div class="name">
                ...
                <div class="center">
                  <milli-input v-model="salesorder.adjustmentFee" :disabled="true" :to-fixed-num="2" class="input" />
                </div>
              </div>
              <div class="item bg-c2">
                <div class="name">
...
<script>
import { getDetail } from '@/api/sale/salesorder'
...
export default {
  ...
  async created() {
    const res = await getPageConfig('salesorder', 'detail')
    if (res && res.success) {
  ...
  methods: {
    async fetchData() {
      const res = await getDetail(this.salesorderId)
      if (res && res.success) {
        ...
        this.$emit('load', this.salesorder)
      }
