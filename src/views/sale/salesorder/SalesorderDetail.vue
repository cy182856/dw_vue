<template>
  <!-- 明细画面 -->
  <div class="summary">
    <salesorder-info :id="id" :key="id" :type="action?action:'detail'" @load="handleLoad" />
    <div class="detail">
      <ul class="operation-box dialog-btn-box dialog-btn-box-detail">
        ...
        <!-- 编辑 -->
        <li v-if="salesorder.workflowFlag === '0' && salesorder.lastFlag === 1 " class="operation-item" @click="handleEdit()">
        ...
        <!-- 删除 -->
        <li v-if="salesorder.workflowFlag === '0' && salesorder.lastFlag === 1" class="operation-item" @click="handleDelete()">
        ...
        <!-- 审批履历 -->
        <li v-if="salesorder.workflowProcessInstId" class="operation-item" @click="approveHisDialog=true">
    ...
    <div class="versionDialog">
      ...
      >
        ...
        <version-his :key="salesorder.identiferNum" func-div="salesorder" :identifer-num="salesorder.identiferNum" @versionClick="versionDialog=false" />
      </el-drawer>
    </div>
    <div v-if="approveHisDialog" class="approveHis">
      <process-detail :key="salesorder.workflowProcessInstId" :title="$t('component.taskApprove.detailedProcess') + salesorder.workflowProcessInstId" :is-show="approveHisDialog" :process-inst-id="salesorder.workflowProcessInstId" @close="approveHisDialog=false" />
    </div>
  </div>
</template>
<script>
import { deleteSalesorder, cancelProcess, deleteApply } from '@/api/sale/salesorder.js'
import SalesorderInfo from '@/views/sale/salesorder/components/SalesorderInfo'
...
export default {
  ...
  methods: {
    handleLoad(salesorder) {
      this.salesorder = salesorder
    },
    handleEdit() {
      this.$router.push({ path: '/sale/salesorder/edit', query: { id: this.id }})
    },
    handleCopy() {
      this.$router.push({ path: '/sale/salesorder/edit', query: { id: this.id, copy: true }})
    },
    handleDelete() {
      this.salesorder.submitType = 'submit'
      this.$confirm(this.$t('comm.tip1'), this.$t('comm.tips'), {
        confirmButtonText: this.$t('comm.certain'),
        cancelButtonText: this.$t('comm.cancel'),
        type: 'warning'
      }).then(async() => {
        const res = await deleteSalesorder([this.salesorder])
        if (res && res.success) {
    ...
    handleSubmit() {
      this.salesorder.submitType = 'deleteReSubmit'
      ...
      this.$confirm(this.$t('comm.tip9'), this.$t('comm.tips'), {
        ...
        deleteApply(this.salesorder).then(() => {
          this.$router.push('/sale/salesorderList')
        })
      }).catch(_ => {
        this.$message({
