  ...
  <div class="apply">
        ...
        <el-form ref="form" class="search-form default-input" label-position="left" :model="searchParam.params">
          <el-form-item prop="identiferNum">
            <el-row type="flex" :gutter="5" justify="space-between">
              <el-col :span="8">
                {{ $t('sale.salesorder.identiferNum') }}:
              </el-col>
              <el-col :span="16">
          ...
          <el-form-item prop="salesorderName">
            <el-row type="flex" :gutter="5" justify="space-between">
              <el-col :span="8">
                {{ $t('sale.salesorder.salesorderName') }}:
              </el-col>
              <el-col :span="16">
                <el-input v-model="searchParam.params.salesorderName" clearable />
              </el-col>
            </el-row>
          </el-form-item>
          ...
          <el-form-item prop="quoteName">
            <el-row type="flex" :gutter="5" justify="space-between">
              <el-col :span="8">
                {{ $t('sale.salesorder.quoteName') }}:
              </el-col>
              <el-col :span="16">
                <el-input v-model="searchParam.params.quoteName" clearable />
              </el-col>
            </el-row>
          </el-form-item>
          <el-form-item prop="contactFullName">
            <el-row type="flex" :gutter="5" justify="space-between">
              <el-col :span="8">
                {{ $t('sale.salesorder.contactFullName') }}:
              </el-col>
              <el-col :span="16">
                <el-input v-model="searchParam.params.contactFullName" clearable />
              </el-col>
            </el-row>
          </el-form-item>
          <el-form-item prop="orderStatus">
            <el-row type="flex" :gutter="5" justify="space-between">
              <el-col :span="8">
                {{ $t('sale.salesorder.orderStatus') }}:
              </el-col>
              <el-col :span="16">
                <dict-select v-model="searchParam.params.orderStatus" dict-type-id="OrderStatus" size="small" class="input" clearable />
              </el-col>
            </el-row>
          </el-form-item>
          <el-form-item prop="assignedUser">
            <el-row type="flex" :gutter="5" justify="space-between">
              <el-col :span="8">
                {{ $t('sale.salesorder.assignedUserName') }}:
              </el-col>
              <el-col :span="16">
        ...
        <div ref="tables" style="height: 100%;">
          ...
          >
            <el-table-column
              ...
              width="55"
            />
            <el-table-column prop="identiferNum" :label="$t('sale.salesorder.identiferNum')" min-width="150">
              <template slot-scope="scope">
                <router-link :to="{path:'/sale/salesorder/detail',query:{id:scope.row.id}}">
                  {{ scope.row.identiferNum }}
                </router-link>
              </template>
            </el-table-column>
            <el-table-column prop="salesorderName" :label="$t('sale.salesorder.salesorderName')" width="200" show-overflow-tooltip />
            <el-table-column :label="$t('sale.salesorder.quoteName')" width="200" show-overflow-tooltip>
              <template slot-scope="scope">
                {{ scope.row.quoteNum | funcName('quote') }}
              </template>
            </el-table-column>
            <el-table-column prop="accountNum" :label="$t('sale.salesorder.accountNum')" width="150" show-overflow-tooltip />
            <el-table-column :label="$t('sale.salesorder.accountName')" width="200" show-overflow-tooltip>
              <template slot-scope="scope">
                {{ scope.row.accountNum | funcName('account') }}
              </template>
            </el-table-column>
            <el-table-column :label="$t('sale.salesorder.contactFullName')" width="200" show-overflow-tooltip>
              <template slot-scope="scope">
                {{ scope.row.contactNum | funcName('contact') }}
              </template>
            </el-table-column>
            <el-table-column :label="$t('sale.salesorder.orderStatus')" min-width="100">
              <template slot-scope="scope">
                <dict-write dict-type-id="OrderStatus" :value="scope.row.orderStatus" />
              </template>
            </el-table-column>
            <el-table-column :label="$t('sale.salesorder.assignedUserName')" min-width="130">
              <template slot-scope="scope">
                {{ scope.row.assignedUser | userName }}
              </template>
            </el-table-column>
            <el-table-column prop="addressDetail" :label="$t('sale.salesorder.contactAddress')" width="240" show-overflow-tooltip />
          </el-table>
        </div>
      </div>
    ...
    <div class="exportDialog">
      <el-dialog
        ...
        width="50%"
      >
        <export-fields-select func-div="salesorder" file-type="xlsx" :search-param="searchParam" />
      </el-dialog>
    </div>
    <div class="historyDialog">
      ...
      >
        ...
        <update-his :key="identiferNum" func-div="salesorder" :identifer-num="identiferNum" />
      </el-drawer>
    </div>
  </div>
</template>
...
import { getList, deleteSalesorder } from '@/api/sale/salesorder.js'
...
export default {
  ...
  methods: {
    ...
    handleCreate() {
      this.$router.push('/sale/salesorder/edit')
    },
    handleCopy() {
      if (this.multipleSelection && this.multipleSelection.length === 1) {
        this.multipleSelection.forEach((rowData) => {
          this.$router.push({ path: '/sale/salesorder/edit', query: { id: rowData.id, copy: true }})
        })
      } else {
        this.$message({
          type: 'info',
    ...
    handleEdit() {
      if (this.multipleSelection && this.multipleSelection.length === 1) {
        ...
        this.$router.push({ path: '/sale/salesorder/edit', query: { id: this.multipleSelection[0].id }})
      } else {
        this.$message({
          type: 'info',
          message: this.$t('comm.msg3')
    ...
    batchDelete() {
      if (this.multipleSelection.length > 0) {
        ...
        this.$confirm(this.$t('comm.tip2'), this.$t('comm.tips'), {
          ...
          deleteSalesorder(this.multipleSelection).then(() => {
            this.search()
          })
        }).catch(() => {
          this.$message({
