  ...
  <div class="apply">
    <div class="search-left-box search-left-box-light">
      ...
      <el-scrollbar>
        <el-form ref="form" class="search-form default-input" label-position="left" :model="searchParam.params">
          ...
          <el-form-item prop="quoteTerm">
            <el-row type="flex" :gutter="5" justify="space-between">
              <el-col :span="8">
                {{ $t('sale.quote.quoteTerm') }}:
              </el-col>
              <el-col :span="16">
                <el-input v-model="searchParam.params.quoteTerm" clearable />
              </el-col>
            </el-row>
          </el-form-item>
          ...
          >
            ...
            <el-table-column prop="identiferNum" :label="$t('sale.quote.identiferNum')" min-width="150">
              <template slot-scope="scope">
                <router-link :to="{path:'/sale/quote/detail',query:{id:scope.row.id}}">
                  {{ scope.row.identiferNum }}
                </router-link>
              </template>
            </el-table-column>
            <el-table-column prop="quoteName" :label="$t('sale.quote.quoteName')" width="200" show-overflow-tooltip />
            <el-table-column prop="accountNum" :label="$t('sale.quote.accountNum')" width="150" show-overflow-tooltip />
            ...
            <el-table-column prop="quoteTerm" :label="$t('sale.quote.quoteTerm')" width="100" align="right" show-overflow-tooltip>
              <template slot-scope="scope">
                {{ scope.row.quoteTerm | numFormat(0) }}
              </template>
            </el-table-column>
            <el-table-column :label="$t('sale.quote.assignedUserName')" min-width="130">
              <template slot-scope="scope">
                {{ scope.row.assignedUser | userName }}
              </template>
            </el-table-column>
            <el-table-column prop="totalAmount" :label="$t('sale.quote.totalAmount')" width="120" align="right" show-overflow-tooltip>
              <template slot-scope="scope">
                {{ scope.row.totalAmount | numFormat(2) }}
              </template>
            </el-table-column>
            <el-table-column prop="productTax" :label="$t('sale.quote.productTax')" width="120" align="right" show-overflow-tooltip>
              <template slot-scope="scope">
                {{ scope.row.productTax | numFormat(2) }}
              </template>
            </el-table-column>
            <el-table-column prop="includingTaxAmount" :label="$t('sale.quote.includingTaxAmount')" width="120" align="right" show-overflow-tooltip>
              <template slot-scope="scope">
    ...
    <div class="exportDialog">
      <el-dialog
        ...
        width="50%"
      >
        <export-fields-select func-div="quote" file-type="xlsx" :search-param="searchParam" />
      </el-dialog>
    </div>
    <div class="historyDialog">
      ...
      >
        ...
        <update-his :key="identiferNum" func-div="quote" :identifer-num="identiferNum" />
      </el-drawer>
    </div>
  </div>
</template>
...
import { getList, deleteQuote } from '@/api/sale/quote.js'
...
export default {
  ...
  methods: {
    ...
    handleCreate() {
      this.$router.push('/sale/quote/edit')
    },
    handleCopy() {
