    ...
    <div class="detail">
        ...
        <el-collapse-item name="1">
          ...
          <div class="input_box">
            ...
            <div class="flex3">
              ...
              <div class="input">
                <rel-func-select v-model="bdDesign.sku" :rel-func="'product'" @data="handleData" />
              </div>
            </div>
          </div>
        </el-collapse-item>
        <el-collapse-item name="2">
          ...
          <div class="input_box">
            <div class="flex3">
              ...
              <div class="input">
                <el-date-picker v-model="bdDesign.budgetYear" size="small" type="year" value-format="yyyy" />
              </div>
            </div>
          </div>
          <div class="input_box">
            ...
            <div class="flex3">
              <el-button class="editbtn" size="mini" icon="el-icon-circle-plus-outline" @click.stop="handleTrial()">
                试算
              </el-button>
            </div>
          </div>
          ...
          >
            <el-table-column prop="type" label="" st min-width="100" />
            ...
            <el-table-column label="1月" align="center">
              <el-table-column prop="price1" label="单价" width="100" align="right">
                <template slot-scope="scope">
                  <milli-input v-if="scope.row.type === '今年计划'" v-model="scope.row.price1" to-fixed-num="2" />
                  <span v-else>{{ scope.row.price1 | numFormat(2) }}</span>
                </template>
              </el-table-column>
              <el-table-column prop="ratio1" label="比率" width="110" align="right">
                <template slot-scope="scope">
                  <milli-input v-if="scope.row.type === '今年计划'" v-model="scope.row.ratio1" to-fixed-num="1" append="%" />
                  <span v-else>{{ scope.row.ratio1 }}%</span>
                </template>
              </el-table-column>
              <el-table-column prop="number1" label="数量" width="100" align="right">
                <template slot-scope="scope">
                  <milli-input v-if="scope.row.type === '今年计划'" v-model="scope.row.number1" />
                  <span v-else>{{ scope.row.number1 | numFormat(2) }}</span>
                </template>
              </el-table-column>
              <el-table-column prop="money1" label="金额" width="100" align="right">
                <template slot-scope="scope">
                  <milli-input v-if="scope.row.type === '今年计划'" v-model="scope.row.money1" />
                  <span v-else>{{ scope.row.money1 | numFormat(2) }}</span>
                </template>
              </el-table-column>
            </el-table-column>
            <el-table-column label="2月" align="center">
              <el-table-column prop="price1" label="单价" width="100" align="right">
                <template slot-scope="scope">
                  <milli-input v-if="scope.row.type === '今年计划'" v-model="scope.row.price1" />
                  <span v-else>{{ scope.row.price1 | numFormat(2) }}</span>
                </template>
              </el-table-column>
              <el-table-column prop="ratio1" label="比率" width="110" align="right">
      ...
      <ul class="operation-box dialog-btn-box dialog-btn-box-detail">
        ...
        <li class="operation-item" @click="handleSubmit('save')">
          ...
          <span class="operation-text">确定</span>
        </li>
      </ul>
    </div>
    <!-- <div>
...
export default {
  ...
  methods: {
    async fetchData() {
      ...
      for (var i = 1; i < 25; i++) {
        var d = {
          category2: i,
          price1: 3 + i,
          ratio1: (100 * Math.random()).toFixed(1),
          number1: 4 + i,
          money1: 40 + i,
          price2: 3 + i,
          ratio2: (100 * Math.random()).toFixed(1),
          number2: 4 + i,
          money2: 40 + i
        }
        if (i > 12) {
      ...
      list[12].category2 = 'Gross Sales'
      list[13].category2 = 'Discount'
      list[14].category2 = 'Trade Disc'
      list[15].category2 = 'Promo Disc'
      list[16].category2 = 'Return'
      list[17].category2 = 'Rebate'
      list[18].category2 = 'Net Sales'
      list[19].category2 = 'Sp-MK'
      list[20].category2 = 'SP-TMK'
      list[21].category2 = 'EC Ad.'
      list[22].category2 = 'Net net Sales'
      list[23].category2 = '加重平均'
    ...
    CellStyle({ row, column, rowIndex, columnIndex }) {
      ...
      if (columnIndex === 1 && (rowIndex === 1 || rowIndex === 7 || rowIndex === 8 || rowIndex === 4 || rowIndex === 5 || rowIndex === 9 ||
        rowIndex === 13 || rowIndex === 17 || rowIndex === 16 || rowIndex === 19 || rowIndex === 20 || rowIndex === 21
      )) {
        return 'account-text-indent1'
      } else if (columnIndex === 1 && (rowIndex === 2 || rowIndex === 3 || rowIndex === 14 || rowIndex === 15)) {
    ...
    handleData(product) {
      this.bdDesign.brand = 'MERRIES'
      this.bdDesign.SubBrand = 'MERRIES EXTRA DRY'
      this.bdDesign.code = '722862'
      this.bdDesign.brandName = 'MRS-ED NB 90x2 1809'
      this.bdDesign.category1 = '2'
    }
