  ...
  <div class="login">
    ...
    <div class="login-info">
      <div class="language-in">
        <!-- 20200704 By Zhang 本系统第一版暂时不支持国际化 , 未来系统需要多语言配置时，可以恢复这个功能。但是，需要完整的测试
      ...
      >
        ...
        <!-- <el-form-item prop="tenantId">
          <select-by-remote
            ...
            action-url="/api/auth/tenantInfo"
            :placeholder="$t('comm.tenantId')"
            clearable
          />
        </el-form-item> -->
        ...
        <div class="retrieve" style="height: 10px;">
          <!--  20200704 by Zhang 第一版暂时不支持找回密码功能。 此功能需要时，请进行完整的测试。
...
export default {
  ...
  methods: {
    ...
    async next() {
      if (!this.validatorAll('retrie')) {
        return false
      }
      if (this.active === 0) {
        this.btnLoading = true
        const res = await sendMail(this.userName)
        if (res && res.success) {
          this.btnLoading = false
          this.active++
        }
      ...
      this.$refs.ruleForm.validate(async valid => {
        if (valid) {
          ...
          if (res && res.success) {
            ...
            // const requrl = 'https://demo-bsind.huge-vision.com/dwreport/decision/login/cross/domain?fine_username=report_user&fine_password=123456&validity=-2'
