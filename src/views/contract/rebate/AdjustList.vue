        ...
        <el-form ref="form" class="search-form default-input" label-position="left" :model="searchParam.params">
          ...
          <el-form-item>
            <el-row type="flex" :gutter="5" justify="space-between">
              ...
              <el-col :span="16">
                <el-select v-model="searchParam.params.uses" placeholder="请选择" clearable>
          ...
          >
            ...
            <el-table-column prop="identiferNum" :label="'Monitor No.'" min-width="100" align="center">
              <template slot-scope="scope">
                <router-link :to="{path:'/contract/rebate/detail',query:{id:scope.row.id}}">
            ...
            <el-table-column prop="uses" :label="'用途'" min-width="80" align="center" />
            <el-table-column prop="localChainCode" :label="'Localchain Code'" min-width="120" align="center" />
            <el-table-column prop="soldtoCode" :label="'Sold-to Code'" min-width="100" align="center" />
            <el-table-column prop="rebateRate" :label="'反利率'" min-width="90" align="right">
              <template slot-scope="scope">
                {{ scope.row.rebateRate }}%
              </template>
            </el-table-column>
            <el-table-column prop="rebateStartDate" :label="'返利维护起始日'" min-width="130" align="center" />
            <el-table-column prop="rebateEndDate" :label="'返利维护截至日'" min-width="130" align="center" />
            <el-table-column prop="discountRate" :label="'折扣率'" min-width="90" align="right">
              <template slot-scope="scope">
                {{ scope.row.discountRate }}%
              </template>
            </el-table-column>
            <el-table-column prop="discountStartDate" :label="'基本交易折扣起始日'" min-width="130" align="center" />
            <el-table-column prop="discountEndDate" :label="'基本交易折扣截至日'" min-width="130" align="center" />
          </el-table>
        </div>
      </div>
...
export default {
  ...
  methods: {
    async fetchData() {
      ...
      for (var i = 0; i < 9; i++) {
        this.adjustList.push({
          id: i + '',
          identiferNum: 'AV00000' + (i + 1),
          applyUser: users[i % 2],
          applyDate: '2021-03-0' + (i + 1),
          uses: uses[i % 3],
          localChainCode: 'R0' + (i + 1),
          soldtoCode: 'SOD00' + (i + 1),
          rebateRate: 1 + i,
          rebateStartDate: '2021-03-0' + (i + 1),
          rebateEndDate: '2021-04-0' + (i + 1),
          discountRate: 2 + i,
          discountStartDate: '2021-03-0' + (i + 1),
          discountEndDate: '2021-04-0' + (i + 1)
        })
      }
    ...
    handleCreate() {
      this.$router.push('/contract/rebate/edit')
    },
    handleCopy() {
      if (this.multipleSelection && this.multipleSelection.length === 1) {
        this.multipleSelection.forEach((rowData) => {
          this.$router.push({ path: '/contract/rebate/edit', query: { id: rowData.id, copy: true }})
        })
      } else {
        this.$message({
          type: 'info',
    ...
    handleEdit() {
      if (this.multipleSelection && this.multipleSelection.length === 1) {
        this.multipleSelection.forEach((rowData) => {
          ...
          this.$router.push({ path: '/contract/rebate/edit', query: { id: rowData.id }})
        })
      } else {
        this.$message({
          type: 'info',
