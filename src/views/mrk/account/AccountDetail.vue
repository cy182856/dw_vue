<template>
  <!-- 详细画面 -->
  <div class="summary">
    <account-info :id="id" :key="id" :type="action?action:'detail'" @load="handleLoad" />
    <div class="detail">
      <ul class="operation-box dialog-btn-box dialog-btn-box-detail">
        ...
        <!-- 审批履历 -->
        <li v-if="account.workflowProcessInstId" class="operation-item" @click="approveHisDialog=true">
    ...
    <div v-if="approveHisDialog" class="approveHis">
      <process-detail :key="account.workflowProcessInstId" :is-show="approveHisDialog" :title="$t('component.taskApprove.detailedProcess') + account.workflowProcessInstId" :process-inst-id="account.workflowProcessInstId" @close="approveHisDialog=false" />
    </div>
    ...
    <div class="versionDialog">
      ...
      >
        ...
        <version-his :key="account.identiferNum" func-div="account" :identifer-num="account.identiferNum" @versionClick="versionDialog=false" />
      </el-drawer>
    </div>
  </div>
</template>
...
import AccountInfo from '@/views/mrk/account/components/AccountInfo'
import { niceScroll } from '@/mixins/nice-scroll.js'
export default {
  name: 'AccountDetail',
  components: { AccountInfo },
  mixins: [names, permission, niceScroll],
  data() {
    return {
      isLoading: true,
      historyDialog: false,
  ...
  methods: {
    ...
    handleCopy() {
      this.$router.push({ path: '/mrk/account/edit', query: { id: this.id, copy: true }})
    },
    handleEdit() {
      this.$router.push({ path: '/mrk/account/edit', query: { id: this.id }})
    },
    handleDelete() {
      this.$confirm(this.$t('comm.tip9'), this.$t('comm.tips'), {
        ...
        this.account.submitType = 'submit'
        const res = await deleteAccount([this.account])
        if (res && res.success) {
    ...
    handleSubmit() {
      this.account.submitType = 'deleteReSubmit'
