<template>
  <div class="apply">
      ...
      <el-scrollbar>
        ...
        >
          ...
          <el-form-item>
            <el-row type="flex" :gutter="5" justify="space-between">
              <el-col :span="8">
                {{ $t("bgt.feeconfiglist.department") }}
              </el-col>
              <el-col :span="16">
                <dict-select v-model="searchParam.params.submitOrg" dict-type-id="Dept" :filter2="searchParam.params.configYear" placeholder="请选择" :disabled="disabled" />
              </el-col>
            </el-row>
          </el-form-item>
          <el-form-item>
            <el-row type="flex" :gutter="5" justify="space-between">
              <el-col :span="8">
                {{ $t("bgt.feeconfiglist.primaryfee") }}
              </el-col>
              <el-col :span="16">
          ...
          <el-form-item>
            <el-row type="flex" :gutter="5" justify="space-between">
              <el-col :span="8">
                {{ $t("bgt.feeconfiglist.secondaryfeeitem") }}
              </el-col>
              <el-col :span="16">
          ...
          <el-form-item>
            <el-row type="flex" :gutter="5" justify="space-between">
              <el-col :span="8">
                {{ $t("bgt.feeconfiglist.activename") }}
              </el-col>
              <el-col :span="16">
    ...
    <div class="content content-light">
      <ul v-if="btnShow" class="operation-box">
        <li class="operation-item" @click="handleDownload()">
          ...
          /></span>
          <span class="operation-text">{{ $t("bgt.feeconfiglist.downloadtemplate") }}</span>
        </li>
        <li class="operation-item" @click="uploadDialog=true">
          ...
          ><i
            class="iconfont iconbianji"
          /></span>
          <span class="operation-text">{{ $t("bgt.feeconfiglist.upload") }}</span>
        </li>
        <li class="operation-item" @click="addTable">
          ...
          <span class="operation-text">{{ $t("bgt.feeconfiglist.newlyadded") }}</span>
        </li>
        <li class="operation-item" @click="removeList">
          <span class="operation-circle circle-middle-btn btn-light-color bluebg"><i class="iconfont iconbaocun" /></span>
          <span class="operation-text">{{ $t("bgt.feeconfiglist.delete") }}</span>
        </li>
        <!-- 保存 -->
      ...
      <div class="table-bg-box search-list-table">
        <div ref="tables" style="height: 100%;">
          ...
          >
            ...
            <el-table-column prop="submitOrg" :label="$t('bgt.feeconfiglist.department1')" min-width="150">
              <template slot-scope="scope">
                <dict-select v-model="scope.row.submitOrg" dict-type-id="Dept" :filter2="searchParam.params.configYear" :disabled="true" />
              </template>
            </el-table-column>
            <el-table-column prop="finAccountCode" :label="$t('bgt.feeconfiglist.Secondaryfeeitem1')" min-width="300">
              <template slot-scope="scope">
            ...
            <el-table-column prop="discPromName" :label="$t('bgt.feeconfiglist.discountactivityname')" min-width="300">
              <template slot-scope="scope">
            ...
            <el-table-column prop="accounts" :label="$t('bgt.feeconfiglist.agent')" min-width="200">
              <template slot-scope="scope">
                <el-input v-model="scope.row.accounts" type="text" :disabled="scope.row.disabled" @focus="showsSKWPop2(scope.row.submitOrg,scope.$index)" />
              </template>
            </el-table-column>
            <el-table-column :label="$t('bgt.feeconfiglist.product')" min-width="200">
              <template slot-scope="scope">
                <el-input v-model="scope.row.sku" type="text" :disabled="scope.row.disabled" @focus="showsSKWPop(scope.row.submitOrg,scope.$index)" />
              </template>
            </el-table-column>
            <el-table-column prop="num1" :label="'BS/KA'" min-width="120" align="center">
              <template slot-scope="scope">
                <dict-select v-if="scope.row.submitOrg==='STRATEGY-SECTION-2'" v-model="scope.row.channel" dict-type-id="ChannelType" :disabled="scope.row.disabled" @change="(value)=>validator('channel'+scope.$index,value)" />
              </template>
            </el-table-column>
            <el-table-column :label="$t('bgt.feeconfiglist.Proportion')" min-width="120" align="right">
              <template slot-scope="scope">
                <milli-input v-if="scope.row.submitOrg==='STRATEGY-SECTION-2'" v-model="scope.row.estSalesRate" to-fixed-num="2" clearable :disabled="scope.row.disabled" style="width:80%;display:inline-block;" @blur="validator('estSalesRate'+scope.$index)" />
                <span v-if="scope.row.submitOrg==='STRATEGY-SECTION-2'">%</span>
              </template>
            </el-table-column>
            <el-table-column prop="expsType" :label="$t('bgt.feeconfiglist.mode')" min-width="150" align="center">
              <template slot-scope="scope">
            ...
            <el-table-column :label="$t('bgt.feeconfiglist.taxrate')" min-width="130" align="right">
              <template slot-scope="scope">
    ...
    <div>
      <el-dialog
        :title="$t('bgt.feeconfiglist.SeriesorSKU')"
    ...
    <div>
      <el-dialog
        :title="$t('bgt.feeconfiglist.agentselection')"
    ...
    <upload :is-show="uploadDialog" :action="`/api/feeconfig/upload/${searchParam.params.configYear}`" :other-params="otherParams" :accept="accept" @onSuccess="handleUploadSuccess" @onClose="uploadDialog=false" />
  </div>
</template>
<script>
...
import { available, getList, finAccountType, finAccount, downloadFile, deleteList, saveListApi } from '@/api/bgt/feeconfig.js'
...
export default {
  ...
  methods: {
    ...
    async fetchData() {
      ...
      // this.customerList(this.searchParam.params.submitOrg)
      // this.productList(this.searchParam.params.submitOrg)
      // this.finAccountList(this.searchParam.params.submitOrg)
    ...
    // async accountLoading() {
    //   this.fees1 = []
    //   const res = await finAccount()
    //   if (res && res.success) {
    //     const results = res.datas.list
    //     console.log(results)
    //     if (results.length > 0) {
    ...
    async getFixedList(param) {
      ...
      if (res && res.success) {
        ...
        console.log(this.fixedList, results, 'fff')
      ...
      if (this.multipleSelection.length > 0) {
        ...
        for (let s = 0; s < delectList.length; s++) {
          ...
          if (delectList[s].configId !== undefined) {
            const deleteList = this.multipleSelection[s]
            const myDeleteListLie = {}
            myDeleteListLie['id'] = deleteList.configId
            idList.push(myDeleteListLie)
          } else {
            const delectid = delectList[s].id
            idList2.push(delectid)
          }
        }
        console.log(idList, idList2)
        ...
        if (idList.length > 0) {
          this.$confirm('是否确认删除？', this.$t('comm.tips'), {
            ...
            const res = await deleteList(this.searchParam.params.configYear, { jsonList: JSON.stringify(idList) })
            if (res && res.success) {
              if (idList2.length > 0) {
              ...
              if (idList.length > 0) {
                for (let y = 0; y < idList.length; y++) {
                  ...
                  for (let i = 0; i < this.fixedList.length; i++) {
                    ...
                    if (removeId === listId) {
                      // console.log(this.fixedList[i], this.accountList[i], '6778')
                      // delete this.fixedList[i]
                      this.$set(this.fixedList, this.fixedList.splice(i, 1))
                      this.$set(this.accountList, this.accountList.splice(i, 1))
                      this.$set(this.skuList, this.skuList.splice(i, 1))
    ...
    async handleDownload() {
      const res = await downloadFile(this.searchParam.params.configYear)
      if (res && res.success) {
        console.log(res)
      }
    },
    ...
    handleOkClick(type) {
      ...
      if (type === 'customers') {
        ...
        console.log(this.skuList, 'this.skuList', this.csskuList)
        ...
        this.$set(this.fixedList, this.selectsku, this.fixedList[this.selectsku])
        // this.fixedList[this.selectsku]['accounts'] = '选择' + skuNum + '个代理商'
        console.log(this.fixedList[this.selectsku].accounts)
        this.accountList[this.selectsku] = names
        console.log(this.accountList, 'this.accountList', this.csaccountList)
    ...
    handleCancelClick(type) {
      // console.log(type, this.csaccount)
      if (type === 'customers') {
        this.customersDialogShow = false
        this.$refs.customerTree.setCheckedKeys([])
      }
    ...
    showsSKWPop(id, index) {
      ...
      if (this.accountDialogShow === false) {
    ...
    showsSKWPop2(id, index) {
      console.log(id, index, this.fixedList)
      if (!id) {
        this.$message({
          type: 'error',
          message: '请先选择部门'
      ...
      // console.log(index, this.csaccount, this.accountList, this.accountList[index])
    },
    handleOpen() {
      try {
    ...
    save() {
      if (this.multipleSelection.length <= 0) {
        ...
        this.$confirm('当前选中项将被保存，是否确认？', this.$t('comm.tips'), {
          ...
          const res = await saveListApi(this.searchParam.params.configYear, { jsonList: JSON.stringify(newSaveList) })
          if (res) {
            if (res.success) {
              this.getFixedList(this.searchParam)
            }
    ...
    getTreeList(list, name1, name2, name3, code3) {
      ...
      for (let s = 0; s < area1.length; s++) {
        ...
        for (let i = 0; i < list.length; i++) {
          ...
          // console.log(parent, lie, '比对')
          if (s === 0 && name2 === 'feeInputType') {
            // console.log(parent, '一级类目')
          }

          if (parent === lie) {
            // console.log(parent, lie, 688)
            area2Id++
