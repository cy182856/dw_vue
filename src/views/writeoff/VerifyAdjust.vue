<template>
  <div class="apply">
        ...
        >
          ...
          <el-form-item>
            <el-row type="flex" :gutter="5" justify="space-between">
              ...
              <el-col :span="16">
                <dict-select v-model="searchParam.params.submitOrg" dict-type-id="Dept" :filter2="happenYear" :disabled="disabled" @change="submitChange" />
              </el-col>
            </el-row>
          </el-form-item>
          <el-form-item>
    ...
    <div class="content content-light">
      <ul class="operation-box">
        ...
        <li class="operation-item" @click="uploadAdjustDialog=true">
          ...
          /></span>
          <span class="operation-text">核销上传</span>
        </li>
          ...
          >
            ...
            >
              <template slot-scope="scope">
                ...
                <!-- <span v-if="(scope.row.ECicon===true)&&(searchParam.params.submitOrg==='EC-DEPT')" class="el-icon-warning" style="font-size:20px;color:orange;" :title="'精准预估金额超过月度比例，月度基准比例为：'+scope.row.ECiconNum" />
                <span v-if="(scope.row.ECicon2===true)&&(searchParam.params.submitOrg==='EC-DEPT')" class="el-icon-warning" style="font-size:20px;color:orange;" :title="'精准预估金额超过月度比例，累计基准比例为：'+scope.row.ECicon2Num" />
                <span v-if="(scope.row.icon===true)&&(searchParam.params.submitOrg!=='EC-DEPT')" class="el-icon-warning" style="font-size:20px;color:orange;" :title="'精准预估金额超过月度比例，月度基准金额为：'+scope.row.tips1" />
                <span v-if="(scope.row.icon2===true)&&(searchParam.params.submitOrg!=='EC-DEPT')" class="el-icon-warning" style="font-size:20px;color:orange;" :title="'精准预估金额超过年度累计比例，累计基准金额为：'+scope.row.tips2" />
                <span v-if="(scope.row.icon3===true)&&(searchParam.params.submitOrg!=='EC-DEPT')" class="el-icon-warning" style="font-size:20px;color:orange;" :title="scope.row.msg" /> -->
              </template>
            </el-table-column>
            <el-table-column :prop="'expsHappenYm'" :label="'发生年月'" min-width="120" align="center">
              <template slot-scope="scope">
                <!-- <div>
                  <el-date-picker v-model="scope.row.expsHappenYm" size="small" type="year" value-format="yyyy" :clearable="false" :disabled="true" />
                </div> -->
                {{ scope.row.expsHappenYm }}
              </template>
            </el-table-column>
            ...
            <el-table-column :prop="'act_amount'" :label="'促销费精准预估金额'" min-width="160" align="right">
              <template slot-scope="scope">
                {{ scope.row.act_amount|numFormat(2) }}
              </template>
            </el-table-column>
            ...
            <el-table-column :prop="'verify_amount'" :label="'核销金额'" min-width="160" align="right">
              <template slot-scope="scope">
                <template v-if="scope.row.addTableDis&&(!scope.row.disabled)">
                  <milli-input v-model="scope.row.verify_amount" class="input" to-fixed-num="2" @input="(value)=>validator('verify_amount'+scope.$index,value)" />
                  <span :ref="'verify_amount'+scope.$index" data-roles="required,negativeNumber" :data-value="scope.row.verify_amount" class="message" />
                </template>
                <template v-else>
                  <milli-input v-model="scope.row.verify_amount" :disabled="scope.row.disabled" class="input" to-fixed-num="2" @input="(value)=>validator('verify_amount'+scope.$index,value)" @focus="amountPreview(scope.$index,scope.row.verify_amount)" @blur="amountChange(scope.$index,scope.row.verify_amount)" />
                  <span :ref="'verify_amount'+scope.$index" data-roles="required,negativeNumber" :data-value="scope.row.verify_amount" class="message" />
                </template>
              </template>
            </el-table-column>
            <el-table-column :prop="'verify_percent'" :label="'核销费比'" min-width="120" align="right">
              <template slot-scope="scope">
                {{ scope.row.verify_percent|numFormat(2) }}
                <span v-if="scope.row.verify_percent">%</span>
              </template>
            </el-table-column>
            <el-table-column :prop="'forwardInAmount'" :label="'结转进'" min-width="120" align="right">
              <template slot-scope="scope">
                {{ scope.row.forwardInAmount|numFormat(2) }}
              </template>
            </el-table-column>
            <el-table-column :prop="'forwardOutAmount'" :label="'结转出'" min-width="120" align="right">
              <template slot-scope="scope">
                {{ scope.row.forwardOutAmount|numFormat(2) }}
              </template>
            </el-table-column>
            <el-table-column :prop="'payDeductAmount'" :label="'应付扣减'" min-width="120" align="right">
              <template slot="header">
                ...
                >
                  ...
                  <span slot="reference" class="info-popover"><i class="el-icon-info" /></span>
                </el-popover>
              </template>
              <template slot-scope="scope">
                {{ scope.row.payDeductAmount|numFormat(2) }}
              </template>
            </el-table-column>
            <el-table-column :prop="'payAmount'" :label="'应付金额'" min-width="120" align="right">
              <template slot="header">
                应付金额
                <el-popover
                  placement="top"
            ...
            <el-table-column :prop="'payPercent'" :label="'应付金额费比'" min-width="120" align="right">
              <template slot-scope="scope">
                {{ scope.row.payPercent|numFormat(2) }}
                <span v-if="scope.row.payPercent">%</span>
              </template>
            </el-table-column>
            <el-table-column :prop="'realPayDeductAmount'" :label="'实付扣减'" min-width="120" align="right">
              <template slot="header">
                实付扣减
                <el-popover
                  placement="top"
              ...
              <template slot-scope="scope">
                {{ scope.row.realPayDeductAmount|numFormat(2) }}
              </template>
            </el-table-column>
            <el-table-column :prop="'realPayAmount'" :label="'实付金额'" min-width="120" align="right">
              <template slot="header">
                实付金额
                <el-popover
                  placement="top"
              ...
              <template slot-scope="scope">
                {{ scope.row.realPayAmount|numFormat(2) }}
              </template>
            </el-table-column>
          </el-table>
        </div>
    ...
    <upload :is-show="uploadDayDialog" action="/api/verify/adjust/day/upload" :other-params="otherParams" :accept="accept" @onSuccess="handleUploadDaySuccess" @onClose="uploadDayDialog=false" />
    <upload :is-show="uploadMonthDialog" action="/api/verify/adjust/month/upload" :other-params="otherParams" :accept="accept" @onSuccess="handleUploadMonthSuccess" @onClose="uploadMonthDialog=false" />
    <upload :is-show="uploadAdjustDialog" action="/api/verify/adjust/adjust/upload" :other-params="otherParams" :accept="accept" @onSuccess="handleUploadAdjustSuccess" @onClose="uploadAdjustDialog=false" />
    <!-- 汇总弹窗 -->
...
import { getDetailDetail, getList, saveListApi, downloadMonthExcel, downloadDayExcel, downloadFixExcel, downloadChangeExcel, downloadSerailExcel } from '@/api/writeoff/verifyadjusts.js'
...
export default {
  ...
  mixins: [search, names, permission, numFormat, formValidator, finSubjects],
  data() {
    return {
      expand: [],
      k: '',
      ...
      searchParam: {
        ...
        params: {
          //  targetYm:String(new Date().getFullYear()) + '-' + this.doHandleMonth(),   2021-10-25 iv-zhang modified
          targetYm: dateFormat(new Date(new Date().setMonth(new Date().getMonth() - 2)), 'yyyy-MM'),
          custName: '',
          submitOrg: '',
          accountType: '',
          finAccountCode: '',
      ...
      uploadAdjustDialog: false, // 上传文件dialog显示标志位,核销调整
      accept: '.xlsx',
      otherParams: null,
      submitOrg2: '',
      pickerOptions0: {
  ...
  computed: {
    ...
    happenYear: function() {
      return this.searchParam.params.targetYm.substr(0, 4)
    },
    orgIdCom() {
      const d = new Date()
    ...
    disabled() {
      let isSelect = true
      let isFlag = false
      if (this.userInfo.roleList.includes('ROLE_WF_MARKET')) {
        return false
      }
  ...
  async created() {
    ...
    if (day <= 15) {
      this.searchParam.params.targetYm = addMonth(dateFormat(new Date(), 'yyyy-MM'), -2)
    } else {
      this.searchParam.params.targetYm = addMonth(dateFormat(new Date(), 'yyyy-MM'), -1)
    }
  ...
  methods: {
    ...
    addTable() {
      ...
      // if ((data.split('-')[1] === '01')) {
      //   data = (data.split('-')[0] - 1) + '-' + '11'
      // } else if (data.split('-')[1] === '02') {
      //   data = (data.split('-')[0] - 1) + '-' + '12'
      // } else if (data.split('-')[1] === '12') {
      //   data = (data.split('-')[0]) + '-' + (data.split('-')[1] - 2)
      // } else {
      //   data = (data.split('-')[0]) + '-0' + (data.split('-')[1] - 2)
      // }
      const tableRow = {
        'id': Math.random() * 10,
        'idid': '1',
          ...
          // row.isexpandloading = true
            ...
            for (let i = 0; this.strategyList[i].id; i++) {
              if (this.strategyList[i].id === row.id) {
                ...
                const cynum = Number(row.verify_amount) - Number(this.iptdata[i].verify_amount)
                ...
                this.iptdata[i].verify_amount = row.verify_amount
                this.strategyList[i].verify_amount = row.verify_amount
                if (this.iptdata[i].sales_act_amount) {
                  this.strategyList[i].verify_percent = this.toPercent2(row.verify_amount, this.iptdata[i].sales_act_amount)
                }
                break
              }
            }
    ...
    async fetchData() {
      ...
      // if (res && res.success) {
      //   this.searchParam.params.targetYm = res.datas.month
      // }
      if (!this.searchParam.params.targetYm || !this.searchParam.params.submitOrg) {
        this.$message({
          type: 'error',
    ...
    async searchList() {
      if (this.searchParam.params.targetYm) {
        this.estYm = this.searchParam.params.targetYm
    ...
    monthBace() {
      ...
      if (dataList.length > 0) {
        for (let s = 0; s < dataList.length; s++) {
          ...
          const verify_amount = Number(dataList[s].verify_amount)
    ...
    amountPreview(index, val) {
      // this.iptdata[index].verify_amount = val
    },
    totalmonth(index) {
    ...
    amountChange(index, val) {
      if (this.strategyList[index].isexpandloading || (this.searchParam.params.submitOrg === 'EC-DEPT')) {
        ...
        const cynum = Number(val) - Number(this.iptdata[index].verify_amount)
        if (/^\d+(?=\.{0,1}\d+$|$)/.test(val)) {
          if (this.strategyList[index].custDetailList) {
          ...
          this.iptdata[index].verify_amount = val
          this.strategyList[index].verify_amount = val
          if (this.iptdata[index].sales_act_amount) {
            this.strategyList[index].verify_percent = this.toPercent(val, this.iptdata[index].sales_act_amount)
          }
          this.calculation()
          this.monthBace()
        } else {
          this.strategyList[index].verify_amount = this.iptdata[index].verify_amount
        }
      }
      if (this.iptdata[index].sales_act_amount) {
        this.strategyList[index].verify_percent = this.toPercent2(val, this.iptdata[index].sales_act_amount)
      }
    },
    ...
    saveList() {
      if (this.multipleSelection.length <= 0) {
        ...
        // for (let j = 0; j < this.strategyList.length; j++) {
        //   for (let i = 0; i < this.multipleSelection.length; i++) {
        //     if (this.strategyList[j].id === this.multipleSelection[i].id) {
        //       const element = this.multipleSelection[i]
        //       // if (!this.validator('custName' + j, element.custName)) {
        //       //   return false
        //       // }
        //       if (!this.validator('finAccountName' + j, element.finAccountName)) {
        //         return false
        //       }
        //       if (!this.validator('productCategory' + j, element.productCategory)) {
        //         return false
        //       }
        //     }
        //   }
        ...
        // 判断是否是新建数据，如果是，id为空
        for (let j = 0; j < this.strategyList.length; j++) {
          if (this.strategyList[j].idid === '1') {
            this.strategyList[j].id = ''
          }
        }
        ...
        for (let s = 0; s < saveList.length; s++) {
          ...
          // const newsavelie = {}
          // newsavelie['id'] = saveListlie.id
          // newsavelie['submitOrg'] = saveListlie.submitOrg
          // newsavelie['amount'] = saveListlie.verify_amount
          // newSaveList.push(newsavelie)
          newSaveList.push(saveListlie)
        }
    ...
    amountChange2(index1, index2, val) {
      if (/^\d+(?=\.{0,1}\d+$|$)/.test(val)) {
        ...
        if (this.strategyList.length > 0) {
          for (let s = 0; s < this.strategyList.length; s++) {
            if (this.searchParam.params.submitOrg === 'EC-DEPT') {
              if (this.strategyList[s].custDetailList && this.strategyList[s].custDetailList.length > 5) {
                if (((this.todel(this.strategyList[s].custDetailList[5]['month_' + index2])) > (this.strategyList[s].month_base_amount * 10000)) && (this.strategyList[s].month_base_amount !== null)) {
                  this.strategyList[s].ECicon = true
                  // if (this.strategyList[s].ECiconNum) {
                  this.strategyList[s].ECiconNum = this.toPercent1(this.strategyList[s].month_base_amount)
