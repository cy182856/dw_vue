<template>
  ...
  <div class="summary">
    ...
    <advertisingSurvey-info v-if="apply.applyType==='1'" :id="id" :key="id" type="detail" :apply="apply" :disabled="true" />
    <promoter-info v-if="apply.applyType==='2'" :id="id" :key="id" type="detail" :apply="apply" :disabled="true" />
    <sampleApply-info v-if="apply.applyType==='3' || apply.applyType==='4'" :id="id" :key="id" type="detail" :apply="apply" :disabled="true" />
    <reIssue-info v-if="apply.applyType==='5'" :id="id" :key="id" type="detail" :apply="apply" :disabled="true" />
    <return-info v-if="apply.applyType==='6'" :id="id" :key="id" type="detail" :apply="apply" :disabled="true" />
    <seal-use-info v-if="apply.applyType==='7'" :id="id" :key="id" type="detail" :apply="apply" :disabled="true" />
    <div class="detail">
      <ul class="operation-box dialog-btn-box dialog-btn-box-detail">
        ...
        <li v-if="apply.workflowProcessInstId" class="operation-item" @click="approveHisDialog=true">
      ...
      <!-- <el-button v-if="apply.workflowFlag === '0' && apply.lastFlag === 1" type="success" class="iconfont icon-save" size="small" @click="handleEdit">
        编辑
      </el-button> -->
      <!-- <el-button v-if="apply.workflowFlag === '0' && apply.lastFlag === 1" type="danger" class="iconfont icon-del" size="small" @click="handleDelete">
      {{ $t('comm.delete') }}
      </el-button> -->
      ...
      <el-button v-if="apply.workflowProcessInstId" type="primary" class="iconfont icon-shenpi" size="small" @click="approveHisDialog=true">
    ...
    <div class="historyDialog">
      ...
      >
        ...
        <update-his :key="apply.identiferNum" func-div="apply" :identifer-num="apply.identiferNum" />
      </el-drawer>
    </div>
    <div class="versionDialog">
      ...
      >
        ...
        <version-his :key="apply.identiferNum" func-div="apply" :identifer-num="apply.identiferNum" @versionClick="versionDialog=false" />
      </el-drawer>
    </div>
    <div v-if="approveHisDialog" class="approveHis">
      <process-detail :key="apply.workflowProcessInstId" :title="$t('component.taskApprove.detailedProcess') + apply.workflowProcessInstId" :is-show="approveHisDialog" :process-inst-id="apply.workflowProcessInstId" @close="approveHisDialog=false" />
    </div>
  </div>
</template>
<script>
// import { deleteapply, downloadPDF } from '@/api/mgt/apply.js'
  ...
  methods: {
    fetchData() {
      if (this.id === '1') {
        ...
        this.apply.reciveReason1Input = { '1': { moveType: '', financialAccount: '', costCenter: '' },
    ...
    handleEdit() {
      this.$router.push({ path: '/mgt/apply/edit', query: { id: this.id }})
    },
    handleCopy() {
      this.$router.push({ path: '/mgt/apply/edit', query: { id: this.id, copy: true }})
    },
    downloadDialog() {
      // downloadPDF(this.id)
    },
    handleDelete() {
      this.$confirm(this.$t('comm.tip1'), this.$t('comm.tips'), {
        ...
        this.apply.submitType = 'submit'
        // deleteapply([this.apply]).then(() => {
        //   this.$router.go(-1)
        // })
      }).catch(() => {
        this.$message({
